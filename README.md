...
既定モデルは `gemini-2.5-flash`（無料枠: 1日1,500リクエスト、15 RPM）です。
Googleは古いモデルを定期的に廃止するため、`429 Too Many Requests`（`limit: 0`）が出た場合は
モデルが廃止された可能性があります。[Gemini APIのレート制限ページ](https://ai.google.dev/gemini-api/docs/rate-limits)
で現行の無料枠モデルを確認し、リポジトリの Secrets に `GEMINI_MODEL` を追加するか、
`scripts/summarize.py` の既定値を更新してください（例: より高いRPMが必要なら `gemini-2.5-flash-lite`）。
