...
## 書籍カテゴリ（Amazon検索ベースの新刊検出）※現在は無効化中

書籍は固定リストではなく、`type: amazon_search` で「Amazon.co.jpで特定キーワードを検索し、
まだ見たことのないASIN（商品）が出てきたら新着として報告する」という方式を実装していました
（`config/sites.yaml` の `book-amazon-search-testing`）。

**2026年7月28日頃から、Amazon側のボット判定（Akamai）がこの方式に対して本格的に反応するようになり、
4日連続でエラーが継続したため、`config/sites.yaml` でコメントアウトして無効化しました。**
GitHub Actionsだけでなくローカル環境からの実行でも同じチャレンジページ（JavaScript実行が必要な
"bm-verify"付きのインタースティシャルページ）が返ることを確認しており、単純なアクセス元IPの問題ではなく、
恒常的に取得できない状態と判断しています。復旧しているか確認したい場合は、コメントを外して
`python scripts/main.py` をローカルで実行し、`book-amazon-search-testing` がエラーにならず
検索結果を取得できるか試してください（`scripts/crawl.py` はチャレンジページを検知した場合、
検索結果0件を静かに「新着なし」として扱わず、明示的にエラーを出すようにしてあります）。

公式のAmazon Product Advertising API（要Amazonアソシエイト登録）に切り替えるのが、
より安定した代替手段です。

## 既知の制限: 一部サイトがGitHub Actionsからのアクセスをブロックすることがある

いくつかのサイト（例: SHIFT、JCSQE）は、ローカル環境からは正常に取得できるものの、
GitHub Actions上で実行すると `403 Forbidden` になることが確認されています。
...
これらは `config/sites.yaml` でコメントアウトして無効化しています。

Amazon検索（旧 `book-amazon-search-testing`）はこれとは別の原因で、GitHub Actionsに限らず
どの環境から実行してもAkamaiのボット判定チャレンジページが返るようになったため無効化しました
（詳細は上記「書籍カテゴリ」の節を参照）。
